import React from 'react';
import { FaShoppingCart } from 'react-icons/fa';

const IbrahimAliyu = () => {
  const items = [
    { id: 1, title: "Ibrahim Aliyu Home Jersey 2024", imageUrl: 'https://example.com/jersey1.jpg', link: 'https://www.example.com/product/1', price: '$129.99' },
    { id: 2, title: "Signed Ibrahim Aliyu Soccer Ball", imageUrl: 'https://example.com/ball1.jpg', link: 'https://www.example.com/product/2', price: '$299.99' },
    { id: 3, title: "Ibrahim Aliyu Name & Number T-Shirt", imageUrl: 'https://example.com/tshirt1.jpg', link: 'https://www.example.com/product/3', price: '$34.99' },
    { id: 4, title: "Ibrahim Aliyu Signed Match Cleats", imageUrl: 'https://example.com/cleat1.jpg', link: 'https://www.example.com/product/4', price: '$349.99' },
    { id: 5, title: "Ibrahim Aliyu Training Jersey", imageUrl: 'https://example.com/jersey2.jpg', link: 'https://www.example.com/product/5', price: '$119.99' },
  ];

  return (
    <div className='w-full min-h-screen bg-white pt-[150px]'>
      <div className='max-w-4xl mx-auto py-8 px-4 md:px-0'>
        <h1 className='text-4xl sm:text-7xl font-bold text-[#020617] mb-4'>Ibrahim Aliyu</h1>

        {/* Player Image and Bio Section */}
        <div className='mb-6'>
          <img 
            src='data:image/jpeg;base64,' 
            alt='Ibrahim Aliyu' 
            style={{ objectPosition: 'center 20%' }} 
            className='w-full h-[400px] object-cover rounded-lg shadow-md' 
          />
        </div>
        <div className="mb-10 text-center text-sm text-gray-500">
          <p>Creator: Unknown | Credit: Team XYZ | Copyright: 2024 Team XYZ</p>
        </div>

        <div className='text-[#020617] mb-8'>
          <h2 className='text-2xl font-semibold mb-2'>Player Information</h2>
          <ul className='list-disc list-inside'>
            <li><strong>Full Name:</strong> Ibrahim Aliyu</li>
            <li><strong>Born:</strong> January 16, 2002 (Kano, Nigeria)</li>
            <li><strong>Position:</strong> Winger</li>
            <li><strong>Current Team:</strong> Houston Dynamo</li>
            <li><strong>Previous Clubs:</strong> Lokomotiva Zagreb, Footwork FC, Oasis FC</li>
            <li><strong>Youth Career:</strong> Oasis FC, Footwork FC, Lokomotiva Zagreb</li>
          </ul>
        </div>

        {/* Additional Text */}
        <div className='text-[#020617] mb-8'>
          <h2 className='text-2xl font-semibold mb-2'>Ibrahim Aliyu: Rising Star from Nigeria</h2>
          <p>
            Ibrahim Aliyu is a talented Nigerian winger, known for his pace and dribbling skills. Starting his youth career with Oasis FC and Footwork FC, he soon joined Lokomotiva Zagreb, where he spent several years honing his craft in European football. Aliyu's ability to create chances and his flair on the ball made him a standout player in Croatia's top league.
          </p>
          <p>
            In 2023, Aliyu made the move to Major League Soccer, joining the Houston Dynamo, where he continues to showcase his talents on a bigger stage. With his youthful energy and determination, Aliyu is a player to watch for the future of both club and country.
          </p>
        </div>

        {/* Shop Section */}
        <h1 className='text-4xl sm:text-7xl font-bold text-[#020617] mb-4'>Shop Ibrahim Aliyu Merchandise</h1>
        <div className='grid gap-6 grid-cols-1 md:grid-cols-3'>
          {items.map(item => (
            <div key={item.id} className='bg-white border border-gray-300 rounded-lg overflow-hidden shadow-lg'>
              <img className='w-full h-64 object-cover' src={item.imageUrl} alt={item.title} />
              <div className='p-4'>
                <h3 className='font-semibold text-lg text-[#020617]'>{item.title}</h3>
                <p className='text-gray-600'>{item.price}</p>
                <a
  href={item.link}
  target="_blank"
  rel="noopener noreferrer"
  className='mt-4 inline-block bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition'
>
  Buy Now
</a>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default IbrahimAliyu;

import React from 'react';
import { FaShoppingCart } from 'react-icons/fa';

const EzequielPonce = () => {
  const items = [
    { id: 1, title: "Ezequiel Ponce Home Jersey 2024", imageUrl: 'https://example.com/jersey1.jpg', link: 'https://www.example.com/product/1', price: '$129.99' },
    { id: 2, title: "Signed Ezequiel Ponce Soccer Ball", imageUrl: 'https://example.com/ball1.jpg', link: 'https://www.example.com/product/2', price: '$299.99' },
    { id: 3, title: "Ezequiel Ponce Name & Number T-Shirt", imageUrl: 'https://example.com/tshirt1.jpg', link: 'https://www.example.com/product/3', price: '$34.99' },
    { id: 4, title: "Ezequiel Ponce Signed Match Cleats", imageUrl: 'https://example.com/cleat1.jpg', link: 'https://www.example.com/product/4', price: '$349.99' },
    { id: 5, title: "Ezequiel Ponce Training Jersey", imageUrl: 'https://example.com/jersey2.jpg', link: 'https://www.example.com/product/5', price: '$119.99' },
  ];

  return (
    <div className='w-full min-h-screen bg-white pt-[150px]'>
      <div className='max-w-4xl mx-auto py-8 px-4 md:px-0'>
        <h1 className='text-4xl sm:text-7xl font-bold text-[#020617] mb-4'>Ezequiel Ponce</h1>

        {/* Player Image and Bio Section */}
        <div className='mb-6'>
          <img 
            src='data:image/jpeg;base64,' 
            alt='Ezequiel Ponce' 
            style={{ objectPosition: 'center 20%' }} 
            className='w-full h-[400px] object-cover rounded-lg shadow-md' 
          />
        </div>
        <div className="mb-10 text-center text-sm text-gray-500">
          <p>Creator: Unknown | Credit: Team XYZ | Copyright: 2024 Team XYZ</p>
        </div>

        <div className='text-[#020617] mb-8'>
          <h2 className='text-2xl font-semibold mb-2'>Player Information</h2>
          <ul className='list-disc list-inside'>
            <li><strong>Full Name:</strong> Ezequiel Ponce Martínez</li>
            <li><strong>Born:</strong> March 29, 1997 (Rosario, Argentina)</li>
            <li><strong>Position:</strong> Forward</li>
            <li><strong>Current Team:</strong> Houston Dynamo</li>
            <li><strong>Previous Clubs:</strong> Newell's Old Boys, Roma, Granada, Lille, AEK Athens, Spartak Moscow, Elche</li>
            <li><strong>Youth Career:</strong> Newell's Old Boys</li>
          </ul>
        </div>

        {/* Additional Text */}
        <div className='text-[#020617] mb-8'>
          <h2 className='text-2xl font-semibold mb-2'>Ezequiel Ponce: A Dynamic Forward</h2>
          <p>
            Ezequiel Ponce is an Argentine forward known for his strength, skill, and finishing ability. Beginning his career with Newell's Old Boys in Argentina, he later moved to European football where he played for top clubs including Roma, Granada, Lille, and AEK Athens.
          </p>
          <p>
            After successful loan spells, Ponce signed with Spartak Moscow before moving to Spain's Elche and later returning to AEK Athens. In 2024, he made his move to Major League Soccer, joining Houston Dynamo, where he continues to make an impact.
          </p>
        </div>

        {/* Shop Section */}
        <h1 className='text-4xl sm:text-7xl font-bold text-[#020617] mb-4'>Shop Ezequiel Ponce Merchandise</h1>
        <div className='grid gap-6 grid-cols-1 md:grid-cols-3'>
          {items.map(item => (
            <div key={item.id} className='bg-white border border-gray-300 rounded-lg overflow-hidden shadow-lg'>
              <img className='w-full h-64 object-cover' src={item.imageUrl} alt={item.title} />
              <div className='p-4'>
                <h3 className='font-semibold text-lg text-[#020617]'>{item.title}</h3>
                <p className='text-gray-600'>{item.price}</p>
                <a
  href={item.link}
  target="_blank"
  rel="noopener noreferrer"
  className='mt-4 inline-block bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition'
>
  Buy Now
</a>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default EzequielPonce;

import React from 'react';
import { FaShoppingCart } from 'react-icons/fa';

const KelvinYeboah = () => {
  const items = [
    { id: 1, title: "Kelvin Yeboah Home Jersey 2024", imageUrl: 'https://example.com/jersey1.jpg', link: 'https://www.example.com/product/1', price: '$129.99' },
    { id: 2, title: "Signed Kelvin Yeboah Soccer Ball", imageUrl: 'https://example.com/ball1.jpg', link: 'https://www.example.com/product/2', price: '$249.99' },
    { id: 3, title: "Kelvin Yeboah Name & Number T-Shirt", imageUrl: 'https://example.com/tshirt1.jpg', link: 'https://www.example.com/product/3', price: '$39.99' },
    { id: 4, title: "Kelvin Yeboah Signed Match Cleats", imageUrl: 'https://example.com/cleat1.jpg', link: 'https://www.example.com/product/4', price: '$329.99' },
    { id: 5, title: "Kelvin Yeboah Training Jersey", imageUrl: 'https://example.com/jersey2.jpg', link: 'https://www.example.com/product/5', price: '$119.99' },
  ];

  return (
    <div className='w-full min-h-screen bg-white pt-[150px]'>
      <div className='max-w-4xl mx-auto py-8 px-4 md:px-0'>
        <h1 className='text-4xl sm:text-7xl font-bold text-[#020617] mb-4'>Kelvin Yeboah</h1>

        {/* Player Image and Bio Section */}
        <div className='mb-6'>
          <img 
            src='data:image/jpeg;base64,' 
            alt='Kelvin Yeboah' 
            className='w-full h-[400px] object-cover rounded-lg shadow-md' 
          />
        </div>
        <div className="mb-10 text-center text-sm text-gray-500">
          <p> </p>
        </div>

        <div className='text-[#020617] mb-8'>
          <h2 className='text-2xl font-semibold mb-2'>Player Information</h2>
          <ul className='list-disc list-inside'>
            <li><strong>Full Name:</strong> Kelvin Kwarteng Yeboah</li>
            <li><strong>Date of Birth:</strong> 6 May 2000 (Accra, Ghana)</li>
            <li><strong>Position:</strong> Striker</li>
            <li><strong>Height:</strong> 1.82 m (6 ft 0 in)</li>
            <li><strong>Current Team:</strong> Minnesota United</li>
            <li><strong>Youth Teams:</strong> West Ham United, Gozzano</li>
            <li><strong>Senior Teams:</strong> AC London, WSG Tirol, Sturm Graz, Genoa, FC Augsburg (loan), Montpellier (loan), Standard Liège (loan), Minnesota United</li>
            <li><strong>International Career:</strong> Italy U21</li>
          </ul>
        </div>

        {/* Additional Text */}
        <div className='text-[#020617] mb-8'>
          <h2 className='text-2xl font-semibold mb-2'>Kelvin Yeboah: Rising Star in MLS</h2>
          <p>
            Kelvin Yeboah is a Ghanaian-born striker who has made a name for himself across several leagues in Europe before joining Minnesota United in Major League Soccer. Known for his pace, agility, and sharp finishing, Yeboah has become a key figure for his team.
          </p>
          <p>
            Yeboah's career highlights include impressive stints at Sturm Graz and Standard Liège, where he showcased his scoring abilities. His international experience with Italy U21 adds to his versatility and football IQ, making him a standout player in MLS.
          </p>
        </div>

        {/* Shop Section */}
        <h1 className='text-4xl sm:text-7xl font-bold text-[#020617] mb-4'>Shop Kelvin Yeboah Merchandise</h1>
        <div className='grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3'>
          {items.map(item => (
            <div key={item.id} className='group rounded-lg overflow-hidden shadow-md hover:shadow-xl transition duration-300'>
              <div className='relative overflow-hidden bg-gray-200 h-48'>
                <img src={item.imageUrl} alt={item.title} className='object-contain w-full h-full group-hover:scale-105 transition duration-300' />
              </div>
              <div className='p-4'>
                <h3 className='text-xl font-bold group-hover:text-blue-600 transition duration-300'>{item.title}</h3>
                <p className='text-lg font-semibold'>{item.price}</p>
                <a href={item.link} target='_blank' rel='noopener noreferrer' className='block bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded transition duration-300 mt-2'>
                  Buy on Amazon <FaShoppingCart className='inline-block ml-2' />
                </a>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default KelvinYeboah;

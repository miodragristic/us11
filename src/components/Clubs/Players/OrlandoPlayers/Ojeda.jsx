import React from 'react';

const MartinOjeda = () => {
  const items = [
    { id: 1, title: "Martín Ojeda Home Jersey", imageUrl: 'https://example.com/jersey.jpg', link: 'https://www.example.com/product/1', price: '$129.99' },
    { id: 2, title: "Signed Ojeda Soccer Ball", imageUrl: 'https://example.com/ball.jpg', link: 'https://www.example.com/product/2', price: '$249.99' },
    { id: 3, title: "Martín Ojeda Poster", imageUrl: 'https://example.com/poster.jpg', link: 'https://www.example.com/product/3', price: '$19.99' },
    { id: 4, title: "Orlando City Scarf", imageUrl: 'https://example.com/scarf.jpg', link: 'https://www.example.com/product/4', price: '$29.99' },
    { id: 5, title: "Martín Ojeda Autographed Photo", imageUrl: 'https://example.com/photo.jpg', link: 'https://www.example.com/product/5', price: '$99.99' },
  ];

  return (
    <div className='w-full min-h-screen bg-gray-100 pt-[150px]'>
      <div className='max-w-4xl mx-auto py-8 px-4 md:px-0'>
        <h1 className='text-4xl sm:text-7xl font-bold text-[#020617] mb-4'>Martín Ojeda</h1>

        {/* Player Image and Bio Section */}
        <div className='mb-6'>
          <img 
            src='data:image/jpeg;base64,' 
            alt='Martín Ojeda' 
            className='w-full h-[400px] object-cover rounded-lg shadow-md' 
          />
        </div>
        <div className="mb-10 text-center text-sm text-gray-500">
          <p>Creator: Unknown | Credit: Team Orlando City | Copyright: 2025 Team Orlando City</p>
        </div>

        <div className='text-[#020617] mb-8'>
          <h2 className='text-2xl font-semibold mb-2'>Player Information</h2>
          <ul className='list-disc list-inside'>
            <li><strong>Date of Birth:</strong> November 27, 1998 (Gualeguaychú, Entre Ríos, Argentina)</li>
            <li><strong>Position:</strong> Winger</li>
            <li><strong>Height:</strong> 5 ft 8 in (1.73 m)</li>
            <li><strong>Current Team:</strong> Orlando City</li>
            <li><strong>Senior Career:</strong> 
              <ul className='list-disc list-inside ml-5'>
                <li>Ferro Carril Oeste (2016–2017): 43 appearances, 5 goals</li>
                <li>Racing Club (2017–2020): 14 appearances, 0 goals</li>
                <li>Huracán (2019–2020, loan): 9 appearances, 1 goal</li>
                <li>Godoy Cruz (2020–2022): 88 appearances, 29 goals</li>
                <li>Orlando City (2023–present): 68 appearances, 10 goals</li>
              </ul>
            </li>
          </ul>
        </div>

        {/* About Section */}
        <div className='text-[#020617] mb-8'>
          <h2 className='text-2xl font-semibold mb-2'>About Martín Ojeda</h2>
          <p>
            Martín Exequiel Ojeda is an Argentine winger known for his speed, dribbling, and precise crosses. Born in Gualeguaychú, Entre Ríos, Ojeda began his career at Ferro Carril Oeste before moving to Racing Club. After brief stints at Huracán and Godoy Cruz, where he made a notable impact, Ojeda joined Orlando City in 2023. His performances with Godoy Cruz earned him recognition, and he quickly became a valuable asset for Orlando City, continuing to show his skill in the MLS.
          </p>
        </div>

        {/* Shop Section */}
        <h1 className='text-4xl sm:text-7xl font-bold text-[#020617] mb-4'>Shop Martín Ojeda Merchandise</h1>
        <div className='grid gap-6 grid-cols-1 md:grid-cols-3'>
          {items.map(item => (
            <div key={item.id} className='bg-white border border-gray-300 rounded-lg overflow-hidden shadow-lg'>
              <img className='w-full h-64 object-cover' src={item.imageUrl} alt={item.title} />
              <div className='p-4'>
                <h3 className='font-semibold text-lg text-[#020617]'>{item.title}</h3>
                <p className='text-gray-600'>{item.price}</p>
                <a href={item.link} className='text-blue-600 hover:underline'>Buy Now</a>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default MartinOjeda;

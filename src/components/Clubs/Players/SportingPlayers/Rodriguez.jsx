import React from 'react';
import { FaShoppingCart } from 'react-icons/fa';

const MemoRodriguez = () => {
  const items = [
    { id: 1, title: "Memo Rodríguez Home Jersey 2024", imageUrl: 'https://example.com/jersey1.jpg', link: 'https://www.example.com/product/1', price: '$129.99' },
    { id: 2, title: "Signed Memo Rodríguez Soccer Ball", imageUrl: 'https://example.com/ball1.jpg', link: 'https://www.example.com/product/2', price: '$299.99' },
    { id: 3, title: "Memo Rodríguez Name & Number T-Shirt", imageUrl: 'https://example.com/tshirt1.jpg', link: 'https://www.example.com/product/3', price: '$34.99' },
    { id: 4, title: "Memo Rodríguez Signed Match Cleats", imageUrl: 'https://example.com/cleat1.jpg', link: 'https://www.example.com/product/4', price: '$349.99' },
    { id: 5, title: "Memo Rodríguez Training Jersey", imageUrl: 'https://example.com/jersey2.jpg', link: 'https://www.example.com/product/5', price: '$119.99' },
  ];

  return (
    <div className='w-full min-h-screen bg-white pt-[150px]'>
      <div className='max-w-4xl mx-auto py-8 px-4 md:px-0'>
        <h1 className='text-4xl sm:text-7xl font-bold text-[#020617] mb-4'>Memo Rodríguez</h1>

        {/* Player Image and Bio Section */}
        <div className='mb-6'>
          <img 
            src='data:image/jpeg;base64,' 
            alt='Memo Rodríguez' 
            className='w-full h-[400px] object-cover rounded-lg shadow-md' 
          />
        </div>
        <div className="mb-10 text-center text-sm text-gray-500">
          <p>Creator: Unknown | Credit: Team XYZ | Copyright: 2024 Team XYZ</p>
        </div>

        <div className='text-[#020617] mb-8'>
          <h2 className='text-2xl font-semibold mb-2'>Player Information</h2>
          <ul className='list-disc list-inside'>
            <li><strong>Full Name:</strong> José Guillermo Rodríguez</li>
            <li><strong>Date of Birth:</strong> December 27, 1995 (Wharton, Texas, USA)</li>
            <li><strong>Position:</strong> Midfielder</li>
            <li><strong>Height:</strong> 1.73 m (5 ft 8 in)</li>
            <li><strong>Current Team:</strong> Sporting Kansas City</li>
            <li><strong>Previous Teams:</strong> Houston Dynamo, Charleston Battery (loan), Rio Grande Valley FC, LA Galaxy, Austin FC</li>
          </ul>
        </div>

        {/* Additional Text */}
        <div className='text-[#020617] mb-8'>
          <h2 className='text-2xl font-semibold mb-2'>Memo Rodríguez: A Homegrown Talent</h2>
          <p>
            Memo Rodríguez is a versatile midfielder known for his technical skill, work rate, and ability to score crucial goals. A product of the Houston Dynamo academy, Memo began his professional journey with loan stints at Charleston Battery and Rio Grande Valley FC, where he honed his skills.
          </p>
          <p>
            After establishing himself as a key player for Houston Dynamo, Rodríguez had brief spells with LA Galaxy and Austin FC before joining Sporting Kansas City in 2024. His ability to adapt to various roles in midfield makes him a valuable asset to any team.
          </p>
        </div>

        {/* Shop Section */}
        <h1 className='text-4xl sm:text-7xl font-bold text-[#020617] mb-4'>Shop Memo Rodríguez Merchandise</h1>
        <div className='grid gap-6 grid-cols-1 md:grid-cols-3'>
          {items.map(item => (
            <div key={item.id} className='bg-white border border-gray-300 rounded-lg overflow-hidden shadow-lg'>
              <img className='w-full h-64 object-cover' src={item.imageUrl} alt={item.title} />
              <div className='p-4'>
                <h3 className='font-semibold text-lg text-[#020617]'>{item.title}</h3>
                <p className='text-gray-600'>{item.price}</p>
                <a
  href={item.link}
  target="_blank"
  rel="noopener noreferrer"
  className='mt-4 inline-block bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition'
>
  Buy Now
</a>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default MemoRodriguez;

import React from 'react';
import { FaShoppingCart } from 'react-icons/fa';

const RobertVoloder = () => {
  const items = [
    { id: 1, title: "Robert Voloder Home Jersey 2024", imageUrl: 'https://example.com/jersey1.jpg', link: 'https://www.example.com/product/1', price: '$129.99' },
    { id: 2, title: "Signed Robert Voloder Soccer Ball", imageUrl: 'https://example.com/ball1.jpg', link: 'https://www.example.com/product/2', price: '$249.99' },
    { id: 3, title: "Robert Voloder Name & Number T-Shirt", imageUrl: 'https://example.com/tshirt1.jpg', link: 'https://www.example.com/product/3', price: '$39.99' },
    { id: 4, title: "Robert Voloder Signed Match Cleats", imageUrl: 'https://example.com/cleat1.jpg', link: 'https://www.example.com/product/4', price: '$329.99' },
    { id: 5, title: "Robert Voloder Training Jersey", imageUrl: 'https://example.com/jersey2.jpg', link: 'https://www.example.com/product/5', price: '$119.99' },
  ];

  return (
    <div className='w-full min-h-screen bg-white pt-[150px]'>
      <div className='max-w-4xl mx-auto py-8 px-4 md:px-0'>
        <h1 className='text-4xl sm:text-7xl font-bold text-[#020617] mb-4'>Robert Voloder</h1>

        {/* Player Image and Bio Section */}
        <div className='mb-6'>
          <img 
            src='data:image/jpeg;base64,' 
            alt='Robert Voloder' 
            className='w-full h-[400px] object-cover rounded-lg shadow-md' 
          />
        </div>
        <div className="mb-10 text-center text-sm text-gray-500">
          <p>Creator: Unknown | Credit: Team XYZ | Copyright: 2024 Team XYZ</p>
        </div>

        <div className='text-[#020617] mb-8'>
          <h2 className='text-2xl font-semibold mb-2'>Player Information</h2>
          <ul className='list-disc list-inside'>
            <li><strong>Full Name:</strong> Robert Voloder</li>
            <li><strong>Date of Birth:</strong> 9 May 2001 (Frankfurt, Germany)</li>
            <li><strong>Position:</strong> Defender</li>
            <li><strong>Height:</strong> 1.85 m (6 ft 1 in)</li>
            <li><strong>Current Team:</strong> Sporting Kansas City</li>
            <li><strong>Youth Teams:</strong> Eintracht Frankfurt, FSV Frankfurt, SG Rosenhöhe Offenbach, 1. FC Köln</li>
            <li><strong>Senior Teams:</strong> 1. FC Köln II, Maribor (loan), Sporting Kansas City</li>
            <li><strong>International Career:</strong> Bosnia and Herzegovina U17, U18, U19; Germany U19, U20</li>
          </ul>
        </div>

        {/* Additional Text */}
        <div className='text-[#020617] mb-8'>
          <h2 className='text-2xl font-semibold mb-2'>Robert Voloder: A Rising Defensive Talent</h2>
          <p>
            Robert Voloder is a German professional footballer known for his defensive capabilities. Starting his youth career with prestigious clubs like Eintracht Frankfurt and 1. FC Köln, he has shown exceptional promise as a versatile defender.
          </p>
          <p>
            After gaining experience on loan with Maribor, Voloder joined Sporting Kansas City, where he continues to make his mark in Major League Soccer. His international career reflects his dual heritage, representing both Bosnia and Herzegovina and Germany at youth levels.
          </p>
        </div>

        {/* Shop Section */}
        <h1 className='text-4xl sm:text-7xl font-bold text-[#020617] mb-4'>Shop Robert Voloder Merchandise</h1>
        <div className='grid gap-6 grid-cols-1 md:grid-cols-3'>
          {items.map(item => (
            <div key={item.id} className='bg-white border border-gray-300 rounded-lg overflow-hidden shadow-lg'>
              <img className='w-full h-64 object-cover' src={item.imageUrl} alt={item.title} />
              <div className='p-4'>
                <h3 className='font-semibold text-lg text-[#020617]'>{item.title}</h3>
                <p className='text-gray-600'>{item.price}</p>
                <a
  href={item.link}
  target="_blank"
  rel="noopener noreferrer"
  className='mt-4 inline-block bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition'
>
  Buy Now
</a>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default RobertVoloder;

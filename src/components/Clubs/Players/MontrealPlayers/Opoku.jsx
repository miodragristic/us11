import React from 'react';
import { FaShoppingCart } from 'react-icons/fa';

const KwadwoOpoku = () => {
  const items = [
    { id: 1, title: "Kwadwo Opoku Home Jersey 2024", imageUrl: 'https://example.com/jersey1.jpg', link: 'https://www.example.com/product/1', price: '$129.99' },
    { id: 2, title: "Signed Kwadwo Opoku Soccer Ball", imageUrl: 'https://example.com/ball1.jpg', link: 'https://www.example.com/product/2', price: '$249.99' },
    { id: 3, title: "Kwadwo Opoku Name & Number T-Shirt", imageUrl: 'https://example.com/tshirt1.jpg', link: 'https://www.example.com/product/3', price: '$39.99' },
    { id: 4, title: "Kwadwo Opoku Signed Match Cleats", imageUrl: 'https://example.com/cleat1.jpg', link: 'https://www.example.com/product/4', price: '$329.99' },
    { id: 5, title: "Kwadwo Opoku Training Jersey", imageUrl: 'https://example.com/jersey2.jpg', link: 'https://www.example.com/product/5', price: '$119.99' },
  ];

  return (
    <div className='w-full min-h-screen bg-white pt-[150px]'>
      <div className='max-w-4xl mx-auto py-8 px-4 md:px-0'>
        <h1 className='text-4xl sm:text-7xl font-bold text-[#020617] mb-4'>Kwadwo Opoku</h1>

        {/* Player Image and Bio Section */}
        <div className='mb-6'>
          <img 
            src='data:image/jpeg;base64,' 
            alt='Kwadwo Opoku' 
            className='w-full h-[400px] object-cover rounded-lg shadow-md' 
          />
        </div>
        <div className="mb-10 text-center text-sm text-gray-500">
          <p> </p>
        </div>

        <div className='text-[#020617] mb-8'>
          <h2 className='text-2xl font-semibold mb-2'>Player Information</h2>
          <ul className='list-disc list-inside'>
            <li><strong>Full Name:</strong> Kwadwo Opoku</li>
            <li><strong>Date of Birth:</strong> 13 July 2001 (Accra, Ghana)</li>
            <li><strong>Position:</strong> Forward</li>
            <li><strong>Height:</strong> 1.70 m (5 ft 7 in)</li>
            <li><strong>Current Team:</strong> CF Montréal</li>
            <li><strong>Youth Team:</strong> Attram De Visser</li>
            <li><strong>Senior Career:</strong> Attram De Visser, Los Angeles FC, CF Montréal</li>
          </ul>
        </div>

        {/* Additional Text */}
        <div className='text-[#020617] mb-8'>
          <h2 className='text-2xl font-semibold mb-2'>Kwadwo Opoku: From Ghana to CF Montréal</h2>
          <p>
            Kwadwo Opoku, a forward from Accra, Ghana, has showcased his talent in various leagues, first rising through the ranks with Attram De Visser before making an impact in Major League Soccer with Los Angeles FC. His impressive performances led him to CF Montréal in 2023, where he continues to evolve into a key player in their attack.
          </p>
          <p>
            Known for his pace and technical ability, Opoku's career continues to grow as he represents both his club and country. With a bright future ahead, he's certainly one to watch in the upcoming seasons.
          </p>
        </div>

        {/* Shop Section */}
        <h1 className='text-4xl sm:text-7xl font-bold text-[#020617] mb-4'>Shop Kwadwo Opoku Merchandise</h1>
        <div className='grid gap-6 grid-cols-1 md:grid-cols-3'>
          {items.map(item => (
            <div key={item.id} className='bg-white border border-gray-300 rounded-lg overflow-hidden shadow-lg'>
              <img className='w-full h-64 object-cover' src={item.imageUrl} alt={item.title} />
              <div className='p-4'>
                <h3 className='font-semibold text-lg text-[#020617]'>{item.title}</h3>
                <p className='text-gray-600'>{item.price}</p>
                <a href={item.link} className='text-blue-600 hover:underline'>Buy Now</a>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default KwadwoOpoku;
